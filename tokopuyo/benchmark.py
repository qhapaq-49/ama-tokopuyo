...
def run_one(seed, width, depth, n_moves, binary=BINARY):
    ...
    for move_num in range(1, n_moves + 1):
        ...
        field, chain_count, move_score, step_scores = pop_chains(field)
        ...
        records.append({
            ...
            "max_chain_step": max(step_scores) if step_scores else 0,
...
def summarize(all_records, widths, depths, seeds):
    ...
    print("\n" + "="*80)
    print(f"{'width':>6} {'depth':>5} | {'max連鎖(平均)':>12} {'max連鎖(max)':>12} {'累計スコア(平均)':>16} {'avg ms/手':>10}")
    print("-"*80)
    ...
    for w in widths:
        for d in depths:
            ...
            max_max_chain  = max(max_chains_per_seed)
            avg_score      = sum(final_scores_per_seed) / len(final_scores_per_seed)
            avg_ms         = sum(avg_ms_per_seed) / len(avg_ms_per_seed)
            print(f"{w:>6} {d:>5} | {avg_max_chain:>12.1f} {max_max_chain:>12} {avg_score:>16,.0f} {avg_ms:>10.0f}")
            summary_rows.append((w, d, avg_max_chain, max_max_chain, avg_score, avg_ms))
...
def main():
    ...
    total = len(tasks)
    print(f"タスク数: {total}  (seeds={args.seeds}, widths={args.widths}, depths={args.depths}, moves={args.moves})")
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"[{done:3d}/{total}] seed={seed} w={width:5d} d={depth:3d}  "
                      f"max連鎖={max_chain}  累計スコア={final_score:>10,}  経過={elapsed:.0f}s")
            except Exception as e:
    ...
    summarize(all_records, args.widths, args.depths, args.seeds)
